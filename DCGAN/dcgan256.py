...
Images = []
images_path = os.getcwd() + '/image'
files = os.listdir(images_path)
for n in range(1, len(files)):
    temp1 = Image.open(f"image/{str(n).zfill(5)}.png")
...
class Model_GAN(object):
    ...
    def eval2(self, num = 0):
        ...
        #x = Image.fromarray(np.uint8(r1*255))
        
        for i in range(len(c)):
            x = Image.fromarray(np.uint8(c[i]*255))
            x.save("Results/i"+str(num)+str(i)+".png")
            
    def final_eval(self, num = 0):
        ...
        for i in range(len(c)):
            x = Image.fromarray(np.uint8(c[i]*255))
            x.save("Results/Evals"+str(num)+str(i)+".png")
